...
# jemalloc option doesn't do anythin in leap
LOCK=$1
N_TEST=$2
N_ELEM=$3
VERSION=$4
...
if [[ $VERSION == "a" ]]
    then 
        gcc bench_c.c libexpht.a -g -ggdb -Og -DDEBUG=1 -lpthread -o bench_debug
    else 
        if [[ $VERSION == "b" ]]
            then
                gcc bench_c.c libexpht.a -g -ggdb -Og -DDEBUG=1 -DVERSION=1 -lpthread -o bench_debug
            else 
                gcc bench_c.c libexpht.a -g -ggdb -Og -DDEBUG=1 -DVERSION=2 -lpthread -o bench_debug
        fi
fi
...
do
    ...
    FILE="../tests/${N_TH}_${N_ELEM}_${LOCK}_${VERSION}_${JEMALLOC}.csv"
