...
// locks are outside of the hashtable structure
...
access* create_acess(int64_t s, int64_t n_threads){
    ...
    entry->header.locks_size = n_threads*3;
    entry->header.thread_id = 0;
    LOCK_INIT( &entry->header.lock, NULL);

    entry->locks = (lock_bucket*) malloc(sizeof(lock_bucket)*n_threads*3);
    ...
    //entry->locks = (lock_bucket*) malloc(sizeof(lock_bucket)*n_threads*3);
    //printf("Lock Spacing: \n");
    for(int64_t i = 0; i<n_threads*3; i++) {
        //(&entry->lock_bucket)[i] = (void*)h;
        //printf("init lock at %ld : %p \n",i,&entry->locks[i]);

        LOCK_INIT(&entry->locks[i].lock, NULL);
        //printf("%d -> %p \n",i,&entry->locks[i].lock);
    }
...
// non-blocking insert for hashexpansion with bulk update at the end
...
hashtable* HashExpansion(hashtable* b,access* pointer, int64_t id_ptr) {
    hashtable* oldB = b;
    ...
    *node_count = 0;
    ...
    while (i < oldK) {
        ...
        lock_hash = hash(i,pointer->header.locks_size);
        WRITE_LOCK(&pointer->locks[lock_hash].lock);
        ...
        UNLOCK(&pointer->locks[lock_hash].lock);
...
hashtable* find_bucket_read(access* entry,size_t value) {
    hashtable* b = entry->ht;
    size_t h = value & (b->header.n_buckets)-1;
    size_t lock_hash = hash(h,entry->header.locks_size);

    LOCKS* bucket_lock = &entry->locks[lock_hash].lock;
    ...
    while(is_bucket_array((&b->bucket)[h].first)) {
        ...
        lock_hash = hash(h,entry->header.locks_size);

        //old = bucket_lock;
        bucket_lock = &entry->locks[lock_hash].lock;
        READ_LOCK(bucket_lock);
        //UNLOCK(old);  
    }
...
hashtable* find_bucket_write(access* entry,size_t value) {
    hashtable* b = entry->ht;
    size_t h = value & (b->header.n_buckets)-1;
    size_t lock_hash = hash(h,entry->header.locks_size);
...
int64_t search(access* entry,size_t value, int64_t id_ptr) {
    ...
    hashtable* b = find_bucket_read(entry,value);
    size_t h = value & (b->header.n_buckets)-1;
    //hash(value,b->header.n_buckets);
    size_t lock_hash = hash(h,entry->header.locks_size);
...
int64_t insert(access* entry, node* n, int64_t id_ptr) {
    ...
    hashtable* b = find_bucket_write(entry,value);
...
int64_t main_hash(access* pointer,size_t value, int64_t id_ptr) {
    ...
    if((chain_size > TRESH) && (b->header.n_ele > b->header.n_buckets)) { 
            ...
            UNLOCK(&b->header.lock);
            hashtable* oldb = b;
            b = HashExpansion(b,pointer,id_ptr);
